...
function removeFromSlot(removeCube, dontSave) {
    var cube = removeCube;
    addThrowable(cube);
...
function removeSlotColorChange(cube){
    cube.css({"color":""});
...
function generate3DBlocks(minValue, maxValue){
...
function loadBasicInteractive() {
    ...
    page0.append(slotContainer).append(instructionOverlay).append(cubesContainer)
...
///==============================================================================================
        ...
        if (!bool) {
		    ...
		    this.bodies[index].Freeze();
                ...
                run: function() {
                    var _this = this;
                    this.isRunning = true;
                    _this.runInterval = setInterval(function() {
                      _this.loop();
                    },25);
                },
                // Make box
                ...
                mouseDrag: function() {
                    ...
                    // mouse press
                    if (this.isMouseDown && !this.mouseJoint && this.isActive) {
    ...
    $.fn.throwable = function(options) {
        if ($.isFunction(this.each)) {
            ...
            var throwableInstance;
	    if ($.throwables.length <= 0) throwableInstance = new throwable();
	    else{
	     throwableInstance = $.throwables[0];
	     options = null;
	    }
            var isEnvSet=false;
            var rt = this.each(function() {
			   ...
			   if($.throwables.length <= 0){
			    ...
			    for (var indx=0; indx<throwableInstance.elements.length; indx++) {
			     if (throwableInstance.elements[indx] == this) alreadyExists = true;
			    }
			    if (!alreadyExists) throwableInstance.elements.push(this);
			   }
                        }
